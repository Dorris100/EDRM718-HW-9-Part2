...
```{r}
sexabuse_resid <- resid(ptsd~csa)
sexabuse_resid_plot <- plot(sexabuse$ptsd, sexabuse_resid, 
                            ylab = "residuals", xlab = "PTSD",
                            main = "PTSD Scores")
```
...
```{r}
sad_qq <- qqnorm(sexabuse_resid, y=sexabuse$ptsd, xlab = "CSA", ylab = "Residual", plot.it = TRUE)
```
...
#9#9 Construct the ANCOVA table after modeling the response variable as a function of theexplanatory variable and covariate (without interaction).
```{r}
